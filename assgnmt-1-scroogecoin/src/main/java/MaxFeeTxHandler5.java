...
public class MaxFeeTxHandler5 {
     ...
     * constructor.
     */
    public MaxFeeTxHandler5(UTXOPool utxoPool) {
    ...
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
        ...
        List<TransactionWithFeesCollection> possibleCollection
                = findPossibleCollections(possibleTxs, txPosToFeesMap);
    ...
    private List<TransactionWithFeesCollection> findPossibleCollections(
            Transaction[] possibleTxs,
            Map<Integer, TransactionWithFees> txPosToFeesMap) {
        List<TransactionWithFeesCollection> possibleCollection
                = new ArrayList<>();
    ...
    private boolean removeAcceptedIfNeeded(
        ...
        if (count == transaction.tx.numInputs()) {
            if (sharedSet.isEmpty()) {
                return proceesSharedAndAddIfNeeded(transaction, accepted, rejected, pool,
                        shared);
