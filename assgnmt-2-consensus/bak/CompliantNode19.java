 ...
 * @since 07/22/18
 */
public class CompliantNode19 implements Node {
    ...
    private Map<Integer, List<Set<Transaction>>> followeesToTransactions;
    ...
    public void setFollowees(boolean[] followees) {
        ...
        for (int i = 0; i < followees.length; i++) {
            if (followees[i]) {
                followeesToTransactions.put(i, new ArrayList<>());
            }
        }
    }
    ...
    public void receiveFromFollowees(Set<Candidate> candidates) {
        ...
        followeeToTransactionMap.forEach((k, v) -> {
            List<Set<Transaction>> listOfTxs = followeesToTransactions.get(k);
            int size = listOfTxs.size();
            if (size == 0) {
                listOfTxs.add(v);
            } else if (size == 1) {
                listOfTxs.add(v);
                if (listOfTxs.get(0).equals(v)) {
                    blackListed[k] = true;
                }
            } else if (size == 2) {
                if (listOfTxs.get(1).equals(v) || listOfTxs.get(1).equals(v)) {
                    blackListed[k] = true;
                }
                listOfTxs.set(roundIndex, v);
            }
        });
    }
}
